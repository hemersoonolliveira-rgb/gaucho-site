...
export default function AdminPage() {
  ...
  const [aba, setAba] = useState<'dashboard'|'produtos'|'orcamentos'|'logs'>('dashboard')
  ...
  const [form, setForm] = useState({
    ...
    destaque: false, promocao: '', estoque: '', imagem_url: ''
  })
  ...
  async function carregarDados() {
    ...
    const orcHoje = (o.data || []).filter(x => new Date(x.criado_em).toDateString() === hoje)
    const orcSemana = (o.data || []).filter(x => new Date(x.criado_em) > semana)
    ...
    setMetricas({
      ...
      orcHoje: orcHoje.length,
      orcSemana: orcSemana.length,
  ...
  async function salvarProduto() {
    ...
    if (editando) {
      ...
      await registrarLog('Hemerson', 'admin', 'EDITAR', 'produtos', editando.id,
        `Editou produto: ${form.nome} — Preço: R$${form.preco}`)
    } else {
      const { data } = await supabase.from('produtos').insert({ ...dados, ativo: true }).select().single()
      await registrarLog('Hemerson', 'admin', 'CRIAR', 'produtos', data?.id,
        `Criou produto: ${form.nome} — R$${form.preco}`)
    }
    setFormAberto(false)
    setEditando(null)
  ...
  async function toggleAtivo(p: Produto) {
    ...
    await registrarLog('Hemerson', 'admin', p.ativo ? 'DESATIVAR' : 'ATIVAR', 'produtos', p.id,
      `${p.ativo ? 'Desativou' : 'Ativou'} produto: ${p.nome}`)
    carregarDados()
  }

  async function toggleDestaque(p: Produto) {
    await supabase.from('produtos').update({ destaque: !p.destaque }).eq('id', p.id)
    await registrarLog('Hemerson', 'admin', 'DESTAQUE', 'produtos', p.id,
      `${p.destaque ? 'Removeu' : 'Adicionou'} destaque: ${p.nome}`)
    carregarDados()
  }
  ...
  async function excluirProduto(p: Produto) {
    ...
    await registrarLog('Hemerson', 'admin', 'EXCLUIR', 'produtos', p.id, `Excluiu produto: ${p.nome}`)
    carregarDados()
  }
  ...
  function abrirEditar(p: Produto) {
    setEditando(p)
    setForm({
      nome: p.nome, preco: String(p.preco), categoria: p.categoria,
      destaque: p.destaque, promocao: p.promocao || '', estoque: String(p.estoque),
  ...
  if (!autenticado) return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'#0f1117' }}>
      <div style={{ background:'#1c2030', borderRadius:16, padding:40, width:360, boxShadow:'0 20px 60px rgba(0,0,0,.4)' }}>
        ...
        <button onClick={login} style={{ width:'100%', background:'#E8722A', color:'#fff', border:'none', borderRadius:8, padding:'12px', fontWeight:700, fontSize:14, cursor:'pointer' }}>
          Entrar
        </button>
      </div>
    </div>
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0f1117', color:'#fff', fontFamily:'sans-serif' }}>
      ...
      <div style={{ background:'#1c2030', borderBottom:'1px solid rgba(255,255,255,.06)', padding:'14px 28px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:11, color:'rgba(255,255,255,.4)' }}>Bem-vindo, Hemerson</div>
          </div>
        </div>
        <button onClick={() => setAutenticado(false)} style={{ background:'rgba(255,255,255,.06)', border:'1px solid rgba(255,255,255,.1)', color:'rgba(255,255,255,.6)', borderRadius:6, padding:'6px 14px', cursor:'pointer', fontSize:13 }}>
          Sair
        </button>
      </div>
      ...
      <div style={{ background:'#161b26', borderBottom:'1px solid rgba(255,255,255,.06)', padding:'0 28px', display:'flex', gap:4 }}>
        {(['dashboard','produtos','orcamentos','logs'] as const).map(a => (
          <button key={a} onClick={() => setAba(a)} style={{ background:'none', border:'none', color: aba===a ? '#E8722A' : 'rgba(255,255,255,.5)', borderBottom: aba===a ? '2px solid #E8722A' : '2px solid transparent', padding:'14px 18px', cursor:'pointer', fontWeight:600, fontSize:13, textTransform:'capitalize', transition:'all .2s' }}>
            {a === 'dashboard' ? '📊 Dashboard' : a === 'produtos' ? '📦 Produtos' : a === 'orcamentos' ? '🧾 Orçamentos' : '📋 Logs'}
          </button>
        ))}
        <button onClick={carregarDados} style={{ marginLeft:'auto', background:'rgba(255,255,255,.05)', border:'1px solid rgba(255,255,255,.1)', color:'rgba(255,255,255,.6)', borderRadius:6, padding:'6px 12px', cursor:'pointer', fontSize:12, alignSelf:'center' }}>
          ↻ Atualizar
        </button>
      </div>
      ...
      <div style={{ padding:28, maxWidth:1300, margin:'0 auto' }}>
        ...
        {aba === 'dashboard' && (
          <div>
            <h2 style={{ fontSize:22, fontWeight:700, marginBottom:24, color:'#fff' }}>Dashboard</h2>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))', gap:16, marginBottom:32 }}>
              {[
                ...
                { label:'Total de logs', valor: metricas.totalLogs || 0, icon:'📋', cor:'#8b5cf6' },
              ].map((c, i) => (
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20 }}>
              <div style={{ background:'#1c2030', borderRadius:12, padding:22, border:'1px solid rgba(255,255,255,.06)' }}>
                <h3 style={{ fontSize:15, fontWeight:700, marginBottom:16, color:'#fff' }}>⭐ Produtos mais adicionados ao orçamento</h3>
              ...
              {/* ÚLTIMOS ORÇAMENTOS */}
              <div style={{ background:'#1c2030', borderRadius:12, padding:22, border:'1px solid rgba(255,255,255,.06)' }}>
                <h3 style={{ fontSize:15, fontWeight:700, marginBottom:16, color:'#fff' }}>🧾 Últimos orçamentos</h3>
                {orcamentos.slice(0,5).map((o, i) => (
                  <div key={i} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'8px 0', borderBottom:'1px solid rgba(255,255,255,.04)' }}>
                    <div>
                      <div style={{ fontSize:13, color:'rgba(255,255,255,.8)' }}>{o.cliente_nome || 'Cliente anônimo'}</div>
        ...
        {aba === 'produtos' && (
          <div>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:24 }}>
              <h2 style={{ fontSize:22, fontWeight:700, color:'#fff' }}>Produtos ({produtos.length})</h2>
            ...
            {formAberto && (
              <div style={{ background:'#1c2030', borderRadius:12, padding:24, marginBottom:24, border:'1px solid rgba(232,114,42,.3)' }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
                  {[
                    { label:'Nome do produto *', key:'nome', type:'text', placeholder:'Ex: Caixa D\'Água 1000L' },
                    { label:'Preço (R$) *', key:'preco', type:'number', placeholder:'0,00' },
                    { label:'Estoque (unidades)', key:'estoque', type:'number', placeholder:'0' },
                    { label:'Promoção / Badge', key:'promocao', type:'text', placeholder:'Ex: Mais Vendido' },
                    ...
                    <div key={f.key}>
                      ...
                      <input type={f.type} placeholder={f.placeholder} value={(form as any)[f.key]}
                        ...
                        style={{ width:'100%', background:'rgba(255,255,255,.05)', border:'1px solid rgba(255,255,255,.1)', color:'#fff', borderRadius:8, padding:'10px 12px', fontSize:13, outline:'none', boxSizing:'border-box' }} />
                    </div>
                  ))}
                  <div>
                    <label style={{ fontSize:12, color:'rgba(255,255,255,.5)', display:'block', marginBottom:6 }}>Categoria *</label>
                    <select value={form.categoria} onChange={e => setForm(p => ({ ...p, categoria: e.target.value }))}
                      style={{ width:'100%', background:'#1c2030', border:'1px solid rgba(255,255,255,.1)', color:'#fff', borderRadius:8, padding:'10px 12px', fontSize:13, outline:'none' }}>
                ...
                <div style={{ display:'flex', gap:10, marginTop:20 }}>
                  <button onClick={salvarProduto} style={{ background:'#E8722A', color:'#fff', border:'none', borderRadius:8, padding:'10px 24px', fontWeight:700, cursor:'pointer' }}>
                    {editando ? 'Salvar alterações' : 'Criar produto'}
                  </button>
                  <button onClick={() => setFormAberto(false)} style={{ background:'rgba(255,255,255,.06)', color:'rgba(255,255,255,.6)', border:'1px solid rgba(255,255,255,.1)', borderRadius:8, padding:'10px 20px', cursor:'pointer' }}>
                    Cancelar
                  </button>
                </div>
              </div>
                ...
                <div key={p.id} style={{ background:'#1c2030', borderRadius:10, padding:'14px 18px', border:`1px solid ${p.ativo ? 'rgba(255,255,255,.06)' : 'rgba(239,68,68,.2)'}`, display:'flex', alignItems:'center', gap:16, opacity: p.ativo ? 1 : 0.6 }}>
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                      <span style={{ fontWeight:700, fontSize:14, color:'#fff' }}>{p.nome}</span>
                      ...
                      {p.promocao && <span style={{ background:'rgba(232,114,42,.15)', color:'#E8722A', fontSize:10, padding:'2px 8px', borderRadius:4, fontWeight:700 }}>{p.promocao}</span>}
                  ...
                  <div style={{ display:'flex', gap:6 }}>
                    <button onClick={() => toggleDestaque(p)} title={p.destaque ? 'Remover destaque' : 'Destacar'} style={{ background:'rgba(245,200,66,.1)', border:'1px solid rgba(245,200,66,.2)', color:'#F5C842', borderRadius:6, padding:'6px 10px', cursor:'pointer', fontSize:14 }}>⭐</button>
                    <button onClick={() => abrirEditar(p)} style={{ background:'rgba(59,130,246,.1)', border:'1px solid rgba(59,130,246,.2)', color:'#60a5fa', borderRadius:6, padding:'6px 10px', cursor:'pointer', fontSize:13, fontWeight:700 }}>Editar</button>
                    <button onClick={() => toggleAtivo(p)} style={{ background: p.ativo ? 'rgba(34,197,94,.1)' : 'rgba(239,68,68,.1)', border:`1px solid ${p.ativo ? 'rgba(34,197,94,.2)' : 'rgba(239,68,68,.2)'}`, color: p.ativo ? '#22c55e' : '#ef4444', borderRadius:6, padding:'6px 10px', cursor:'pointer', fontSize:12, fontWeight:700 }}>
                      {p.ativo ? 'Ativo' : 'Inativo'}
                    </button>
                    <button onClick={() => excluirProduto(p)} style={{ background:'rgba(239,68,68,.1)', border:'1px solid rgba(239,68,68,.2)', color:'#ef4444', borderRadius:6, padding:'6px 10px', cursor:'pointer', fontSize:13, fontWeight:700 }}>✕</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        {aba === 'orcamentos' && (
          <div>
            <h2 style={{ fontSize:22, fontWeight:700, marginBottom:24, color:'#fff' }}>Orçamentos ({orcamentos.length})</h2>
              ...
              {orcamentos.map(o => (
                <div key={o.id} style={{ background:'#1c2030', borderRadius:10, padding:'16px 20px', border:'1px solid rgba(255,255,255,.06)' }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:12 }}>
                    <div>
                      <div style={{ fontWeight:700, fontSize:15, color:'#fff' }}>{o.cliente_nome || 'Cliente anônimo'}</div>
        ...
        {aba === 'logs' && (
          <div>
            <h2 style={{ fontSize:22, fontWeight:700, marginBottom:24, color:'#fff' }}>Log de Auditoria ({logs.length})</h2>
            <div style={{ display:'grid', gap:8 }}>
              {logs.map(l => (
                <div key={l.id} style={{ background:'#1c2030', borderRadius:8, padding:'12px 16px', border:'1px solid rgba(255,255,255,.05)', display:'flex', alignItems:'center', gap:14 }}>
                  <div style={{
                    background: l.acao === 'CRIAR' ? 'rgba(34,197,94,.15)' : l.acao === 'EXCLUIR' ? 'rgba(239,68,68,.15)' : l.acao === 'EDITAR' ? 'rgba(59,130,246,.15)' : 'rgba(245,200,66,.15)',
                    color: l.acao === 'CRIAR' ? '#22c55e' : l.acao === 'EXCLUIR' ? '#ef4444' : l.acao === 'EDITAR' ? '#60a5fa' : '#F5C842',
                    borderRadius:6, padding:'3px 10px', fontSize:11, fontWeight:700, minWidth:70, textAlign:'center'
                  }}>{l.acao}</div>
                  <div style={{ flex:1, minWidth:0 }}>
              ...
              {!logs.length && <div style={{ textAlign:'center', padding:60, color:'rgba(255,255,255,.3)' }}>Nenhuma ação registrada ainda</div>}
            </div>
          </div>
        )}
